#!/usr/bin/env bash
# Fast variant: uses build_ozon_rag_index_fast.py with HNSW disabled during bulk.
# Uploads to qdrant_fast.tar.gz (separate from v11 output).
set -euo pipefail
...
try:
    ...
    HfApi(token="${HF_TOKEN}").upload_file(
        ...
        path_in_repo="pod_fast.log",
        repo_id="${HF_REPO_ID}",
        repo_type="dataset",
    )
    print("[pod] log uploaded to HF as pod_fast.log")
...
echo "[pod] === download build_index_fast.py from HF ==="
curl -fsSL \
  -H "Authorization: Bearer ${HF_TOKEN}" \
  "https://huggingface.co/datasets/${HF_REPO_ID}/resolve/main/build_index_fast.py?ts=$(date +%s)" \
  -o build_index.py
ls -l build_index.py

echo "[pod] === run FAST indexer (HNSW disabled, encode_batch=256) ==="
HF_TOKEN="${HF_TOKEN}" python3 build_index.py --index-path /workspace/ozon_rag.qdrant 2>&1

echo "[pod] === tarball ==="
cd /workspace
tar czf qdrant_fast.tar.gz ozon_rag.qdrant
ls -lh qdrant_fast.tar.gz

echo "[pod] === upload qdrant_fast.tar.gz ==="
python3 - <<EOF
from huggingface_hub import HfApi
api = HfApi(token="${HF_TOKEN}")
api.upload_file(
    path_or_fileobj="/workspace/qdrant_fast.tar.gz",
    path_in_repo="qdrant_fast.tar.gz",
    repo_id="${HF_REPO_ID}",
    repo_type="dataset",
    commit_message="qdrant fast variant result",
)
print("[pod] qdrant_fast.tar.gz uploaded")
EOF

echo "[pod] DONE_INDEXING_FAST at $(date)"
sleep 7200
