"""Spawn RunPod pod for datasets_rag ALL datasets (single comprehensive run).

Flow:
  1. Upload runpod_pod_runner_datasets_all.sh (as run.sh) +
     ingest_dataset_to_qdrant.py to HF dataset repo (private).
  2. Spawn a GPU pod; pod runs all 9 dataset ingests, each into its own subdir,
     uploads to HF parquet_all/ with unique names, writes DONE_ALL marker.
  3. Save state to scripts/.runpod_datasets_all_state.json.

Monitor : check HF repo for DONE_ALL file + datasets_pod_all.log
Restore : python scripts/restore_datasets_rag.py --recreate
Terminate: python scripts/runpod_terminate.py --state scripts/.runpod_datasets_all_state.json
"""
...
# 9 datasets, largest is OFF ~400k rows + OBF ~900k + amazon 3 cats: need ~150 GB disk
CONTAINER_DISK_GB = 150

# GPU candidates: prefer 24 GB VRAM (3090/A5000) for full-speed encode; 8 GB ok.
GPU_TYPE_IDS = [
...
def hf_upload_folder(repo_id: str, files: list[tuple[Path, str]]) -> str:
    """Upload multiple files in a single commit. Returns immutable commit SHA."""
    try:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        commit = api.upload_folder(
            ...
            repo_id=repo_id,
            repo_type="dataset",
            commit_message="datasets-rag-all bootstrap scripts",
        )
    sha: str = commit.oid
    for _, dst in files:
...
def main() -> None:
    ...
    runner = PROJECT_ROOT / "scripts" / "runpod_pod_runner_datasets_all.sh"
    ...
    print(f"          look for DONE_ALL file + datasets_pod_all.log")
    print(f"Restore : python scripts/restore_datasets_rag.py --recreate")
    print(f"          (downloads from parquet_all/ — all 9 datasets, unique names)")
    print(f"Terminate: python scripts/runpod_terminate.py --state {STATE_FILE}")
