"""Spawn RunPod pod for datasets_rag Phase-2 indexing.

Flow:
  1. Upload runpod_pod_runner_datasets_p2.sh (as run.sh) +
  ...
  3. Save state to scripts/.runpod_datasets_p2_state.json.

Phase-2 datasets:
  abo          Amazon Berkeley Objects  ~147k home/furniture  cap 150k
  rebrickable  LEGO sets               ~20k                  full
  amazon       Electronics + Home_and_Kitchen + Clothing_Shoes_and_Jewelry  cap 150k each

Parquet shards are uploaded to the same parquet/ folder as Phase-1 (same HF
repo).  restore_datasets_rag.py will download ALL parquet/*.parquet and upsert
all points — Phase-1 (311k) + Phase-2 (~570k) = ~880k total.

Monitor with:  check HF https://huggingface.co/datasets/<repo> for DONE_DATASETS_P2
Restore with:  python scripts/restore_datasets_rag.py
Terminate with: python scripts/runpod_terminate.py --state scripts/.runpod_datasets_p2_state.json
"""
...
STATE_FILE = PROJECT_ROOT / "scripts" / ".runpod_datasets_p2_state.json"
DOCKER_IMAGE = "pytorch/pytorch:2.4.0-cuda12.1-cudnn9-runtime"
CONTAINER_DISK_GB = 80   # model cache + parquet output
...
def hf_upload_folder(repo_id: str, files: list[tuple[Path, str]]) -> str:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        commit = api.upload_folder(
            ...
            repo_id=repo_id,
            repo_type="dataset",
            commit_message="datasets-rag-p2 bootstrap scripts",
        )
    sha: str = commit.oid
    for _, dst in files:
    ...
    for gpu_id in GPU_TYPE_IDS:
        variables = {
            "input": {
                ...
                "minMemoryInGb": 12,
                "gpuTypeId": gpu_id,
                "name": "eg-datasets-rag-indexer-p2",
...
def main() -> None:
    ...
    runner = PROJECT_ROOT / "scripts" / "runpod_pod_runner_datasets_p2.sh"
    ...
    state = {
        ...
        "gpu": gpu_used,
        "job": "datasets_rag_phase2",
    }
    STATE_FILE.write_text(json.dumps(state, indent=2))
    print(f"[State] saved to {STATE_FILE}")
    print()
    print(f"Monitor : check HF https://huggingface.co/datasets/{hf_repo_id}")
    print(f"          look for DONE_DATASETS_P2 file + datasets_pod_p2.log")
    print(f"Restore : python scripts/restore_datasets_rag.py")
    print(f"          (downloads ALL parquet/*.parquet — Phase-1 + Phase-2 combined)")
    print(f"Terminate: python scripts/runpod_terminate.py --state scripts/.runpod_datasets_p2_state.json")
