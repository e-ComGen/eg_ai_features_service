...
# «Объединить на одной карточке» (8292) — SELLER-ONLY merge-ключ. Движок его НЕ
# заполняет (см. post_process_values): продавец/eg_importer сам решает группировку.
...
class OzonStrategy(MarketplaceStrategy):
    ...
    def post_process_values(
        ...
        # (D) «Объединить на одной карточке» (8292) — SELLER-ONLY → no_data.
        # Это merge-ключ группировки: продавец сам решает, какие SKU свести в одну
        # карточку (одинаковое значение = «переключалка», уникальное = не объединять).
        # Из названия/источников это НЕ вывести — любая угадайка (имя товара, «Да»,
        # бренд+модель) либо склеит лишнее, либо просто шум. По решению владельца +
        # eg_importer движок поле НЕ заполняет: снимаем значение из ЛЮБОГО источника,
        # поле уходит в skipped→no_data, ключ проставляет eg_importer (уникальный
        # артикул / своя логика группировки).
        if _CARD_GROUP_ATTR_ID in existing_ids:
            ...
            existing_ids.discard(_CARD_GROUP_ATTR_ID)
            values_by_id.pop(_CARD_GROUP_ATTR_ID, None)
