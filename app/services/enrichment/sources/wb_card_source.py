...
from app.services.enrichment.sources.wb_card_cdn import (
    _BASKET_THRESHOLDS,
    _BASKET_DEFAULT,
    _ALL_BASKET_NN,
    _CHROME_UA,
    _HTTP_TIMEOUT,
    _basket_nn_from_table,
    _card_url,
    try_basket,
    fetch_card,
    extract_options,
...
# _BASKET_THRESHOLDS/_BASKET_DEFAULT/_ALL_BASKET_NN/_CHROME_UA/_HTTP_TIMEOUT перенесены в
# wb_card_cdn.py (S1), импортированы выше для обратной совместимости.
...
class WbCardSource(AttributeSource):
    ...
    async def _fetch_card(
        ...
        """Скачать card.json с CDN (см. wb_card_cdn.fetch_card). Обёртка сохраняет
        self._try_basket как перехватываемую точку для тестов."""
        return await fetch_card(client, nm_id, try_basket_fn=self._try_basket)
    ...
    async def _try_basket(
        ...
        """Один CDN GET (см. wb_card_cdn.try_basket)."""
        return await try_basket(client, nn, nm_id)
    ...
    def _extract_options(card: dict) -> list[dict]:
        """Извлечь characteristics из WB card.json (см. wb_card_cdn.extract_options)."""
        return extract_options(card)
